...
while not keyboard:Pressed(hg.K_Escape) do
	...
	if hg.ImGuiBegin('Transform and Physics', true, hg.ImGuiWindowFlags_AlwaysAutoResize) then
		...
		if hg.ImGuiButton('Press to reset position using Transform.SetPos') then
		...
		if physics:NodeHasBody(cube_node) then
			if hg.ImGuiButton('Press to destroy the cube node physics') then
				physics:NodeDestroyPhysics(cube_node)
				physics:GarbageCollect(scene)
			end
		else
			if hg.ImGuiButton('Create to create the cube node physics') then
