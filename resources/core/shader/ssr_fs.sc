...
void main() {
	...
	// roughness
	float roughness = texture2D(u_attr1, uv * uv_ratio).z;
	...
	for (int i = 0; i < sample_count; ++i) {
		float r = roughness * (float(i) + jitter.y) / float(sample_count);
		float spread = r * 3.141592 * 0.5 * 0.99;
		float cos_spread = cos(spread), sin_spread = sin(spread);

		for (int j = 0; j < sample_count; ++j) {
			float angle = float(j + jitter.w) / float(sample_count) * 2. * 3.141592;
