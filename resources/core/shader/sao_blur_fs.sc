...
void main() {
	...
	vec2 center = vec2(texture2D(u_depth, v_texcoord0).r, linear_depth(texture2D(u_input, v_texcoord0).r));
			
	vec2 acc = vec2(center.x, 1.0);
	for(int i=1; i<=KERNEL_RADIUS/2; i++) {
		vec2 p = v_texcoord0 + float(i) * u_direction;
		vec2 current = vec2(texture2D(u_depth, p).r, linear_depth(texture2D(u_input, p).r));
		float weight = compute_weight(float(i), current.y, center.y);
		acc += vec2(current.x*weight, weight);

		p = v_texcoord0 - float(i) * u_direction;
		current = vec2(texture2D(u_depth, p).r, linear_depth(texture2D(u_input, p).r));
