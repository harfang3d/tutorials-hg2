...
if mon_list_size == 2:
    mode_list = mode_list[:5]
    mode_list_str = mode_list_str[:5]
# If only 1 monitor, remove the second and the third
if mon_list_size == 1:
    mode_list = mode_list[:4]
    mode_list_str = mode_list_str[:4]
...
press_apply = False
while not hg.ReadKeyboard().Key(hg.K_Escape) and hg.IsWindowOpen(config_win) and not press_apply:
    hg.ImGuiBeginFrame(config_res_x, config_res_y, hg.TickClock(), hg.ReadMouse(), hg.ReadKeyboard())

    # Imgui window configutator 
    if hg.ImGuiBegin("Window Configuration", True, hg.ImGuiWindowFlags_NoMove | hg.ImGuiWindowFlags_NoResize):
...
if press_apply:
    # Init preset window
    selected_res = res_list[res_preset]
    selected_res_x, selected_res_y = selected_res[0], selected_res[1]
    selected_window_mode = mode_list[window_mode_preset]
    ...
    while not hg.ReadKeyboard().Key(hg.K_Escape) and hg.IsWindowOpen(win):
        ...
        # Imgui window configutator 
        if hg.ImGuiBegin("Window Parameters", True, hg.ImGuiWindowFlags_NoResize | hg.ImGuiWindowFlags_NoTitleBar) :
            # Set the ui position and size in the window
            ...
            hg.ImGuiText("Window size : {} x {}".format(selected_res_x, selected_res_y))
            hg.ImGuiText("Window mode : {}".format(mode_list_str[window_mode_preset]))
